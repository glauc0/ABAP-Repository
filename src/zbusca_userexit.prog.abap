...
****                      © Consultoria                               *
****                                                                  *
**** PROGRAMA: ZBUSCA_USEREXIT                                        *
****                                                                  *
**** DESCRIÇÃO: Programa que busca USER_EXIT/BADI ENHARECEMENT/OUTROS *
****                                                                  *
**** AUTOR:        Consulting                                         *
**** DATA:         29/Abril/2019                                      *
***********************************************************************
...
report zbusca_userexit no standard page heading.

*  PROGRAMA DE BUSCA TOP BUSCA POR EXIT BADI Enhancements

*&--------------------------------------------------------------------&*
*& Report:       Z_BUSCA_USEREXIT (V10.0)                             &*
*& Last updated: 02 outubro 2009                                      &*
*&--------------------------------------------------------------------&*
* Abaixo segue o titulo para colocar Elemento de texto->texto de seleção
* Retire o (*) copie e acesse o caminho acima

*Formato ALV  P_ALV
*Incluir autoridade verificação pesquisa  P_AUTH
*Display BADIs  P_BADI
*Trans Mostrar eventos de negócios  P_BTE
*BADIs só cliente   P_CUSB
*Mostra classe saídas de desenvolvimento  P_DEVC
*User-Exits	P_EXIT
*Mostra modulos de função	P_FUNC
*Linguagem  P_LANGU
*Sem limite. de apresentar a pesquisa	P_LIMIT
*Formato lista Standard	P_LST
*Nome do Programa	P_PNAME
*Programa Exits	P_PROG
*Mostra Subnivel  P_SUBM
*Código de transação  P_TCODE
*Procurar texto	P_TEXT
*Workflow links de tela	P_WFLOW

*&--------------------------------------------------------------------&*
* Abaixo segue o titulo para colocar Elemento de texto->Simbolo de texto
* Retire o (*) copie e acesse o caminho acima

*Digite tCode ou programa	M01
*Insira os critérios, pelo menos, um escopo	M02
*Seleção de Dados (tCode tem precedência sobre o nome do programa)  S01
*Critérios Scopo  S02
*Critérios Tela	S03
*&--------------------------------------------------------------------&*
...
tables: sxs_attr, " Exit: lado de definição: atributos de uma ampliação
        ...
        trdirt,   " Textos título p/programas em TRDIR
        sxc_exit. " BADI exits->>
       "<<-Exit: lado de implementação: atribuição: exit - implement.
*----------------------------------------------------------------------*
*** Tipos
*----------------------------------------------------------------------*
...
constants:
  ...
  c_devc(60) type c value 'User-exits from development classesFunction',
...
parameter: p_devc  type char1 default ' ' modif id a01,
           p_func  type char1 default ' ' modif id a01,
           p_subm  type char1 default ' ' modif id a01.
...
* TCode does not include program name, but does have reference TCode
      ...
      if sy-subrc = 0.
        ...
        p_pname = wa_tstc-pgmna.
        if sy-subrc <> 0.
          message s110(/saptrx/asc)
          with 'No program found for: ' p_tcode."#EC NOTEXT
          stop.
        endif.
      else.
        message s110(/saptrx/asc)
...
* store development classes
  if p_devc = c_x.
    loop at i_fmodule.
      ...
      select single area
        from enlfdir
        into wa_enlfdir-area
       where funcname = i_fmodule-name.

      check not wa_enlfdir-area is initial.

      select single devclass
        into wa_tadir-devclass
        from tadir
       where pgmid    = 'R3TR'
         and object   = 'FUGR'
...
* check i_stoken for entries
      ...
      if w_linnum gt 0.
        ...
        w_incl = ''.
        perform data_search tables i_stoken
          using w_level w_prog w_incl.
      endif.
    endif.
...
* AUTHORITY-CHECKS
    if p_auth = c_x.
      if p_stoken-str eq 'AUTHORITY-CHECK'.
        check p_level eq '0'. "DoNotPerformForFunctionModules(2nd pass)
        w_index = sy-tabix + 2.
...
* Get the CMOD project name
      ...
      select single
      modact~member modact~name modattr~status
...
* BADI data
    if  i_userexit-type eq 'BADI'.
      ...
      select single exit_name
        into wa_sxs_attr-exit_name
        from sxs_attr
       where exit_name = i_userexit-txt.
    ...
    select name
     ...
     where devclass = wa_tadir-devclass.
      ...
      if sy-subrc <> 0.
        ...
        i_userexit-type     = 'Enhancement'.
        i_userexit-modname  = wa_modsapa-name.
        i_userexit-txt      = 'Determined from program DevClass'.
...
* data selection message to sap gui
  ...
  delete adjacent duplicates from i_userexit
                        comparing txt pname modname.
...
* Create field catalog
    perform create_field_catalog
      using 'TYPE'           'T_USEREXIT' ' ' 'Type'.
    perform create_field_catalog
      using 'PNAME'          'T_USEREXIT' ' ' 'Prog?am name'.
    perform create_field_catalog
      using 'TXT'            'T_USEREXIT' ' ' 'Enhancement'.
    perform create_field_catalog
      using 'LEVEL'          'T_USEREXIT' c_x 'Level'.
    perform create_field_catalog
      using 'MODNAME'        'T_USEREXIT' ' ' 'Enhancement name'.
    perform create_field_catalog
      using 'MODTEXT'        'T_USEREXIT' ' ' 'Enhancement text'.
    perform create_field_catalog
      using 'MODATTR-MEMBER' 'T_USEREXIT' c_x 'Member'.
    perform create_field_catalog
      using 'MODATTR-NAME'   'T_USEREXIT' ' ' 'Project'.
    perform create_field_catalog
      using 'MODATTR-STATUS' 'T_USEREXIT' ' ' 'Status'.
    perform create_field_catalog
      using 'MODATTR-ANAM'   'T_USEREXIT' ' ' 'Changed by'.
    perform create_field_catalog
      using 'MODATTR-ADAT'   'T_USEREXIT' ' ' 'Change date'.
      ...
      case rs_selfield-sel_tab_field.
        when 'T_USEREXIT-MODNAME'.
          ...
          case i_userexit-type.
            ...
            when 'BusTrEvent'.
              submit rfopfi00
                with event = i_userexit-modname(8) and return.
            when others.
